...
class ByteViewSpec extends WordSpec with ShouldMatchers {
  "A byte view" should {
    "have a length of 5 when constructed from a 5 character string" in {
      val view = ByteView("12345")
      view.length should equal (5)
    }

    "have a length of 5 when constructed from an array of 5 bytes" in {
      val bytes = Array[Byte](1, 2, 3, 4, 5)
      val view = ByteView(bytes)
      view.length should equal (5)
    }

    "not equal an object that is not of type byte view" in {
      val test = ByteView("test").equals(2)
      test should equal (false)
    }
  }

  "A byte view containing a string" should {
    "return a copy of the data as a byte slice" in {
      val str = "12345"
    ...
    "have a string representation equal to that string" in {
      val str = "some string"
      val view = ByteView(str)
      view.toString should equal (str)
    }

    "return its second character when at(1) is called" in {
      val view = ByteView("test")
      view.at(1) should equal ('e'.toByte)
    }

    "return its 2-3 characters when slice(1, 3) is called" in {
      val view = ByteView("test_string")
      view.slice(1, 3).toString should equal ("es")
    }

    "return all but its first 2 characters when sliceFrom(2) is called" in {
      val view = ByteView("test_string")
      view.sliceFrom(2).toString should equal("st_string")
    }

    "copy the string to a byte array" in {
      val str = "test_string"
      val view = ByteView(str)
    ...
    "copy as much of the string as it can to a byte array with a short length" in {
      val str = "test_string"
      val view = ByteView(str)
    ...
    "copy all of the string into a byte array larger than the string" in {
      val str = "test"
  ...
  "A byte view containing a byte array" should {
    "return a copy of the data as a byte slice" in {
      val bytes = Array[Byte](1, 2, 3, 4, 5)
      val view = ByteView(bytes)
    ...
    "have a string representation of that byte array" in {
      val bytes = Array[Byte]('t', 'e', 's', 't')
      val view = ByteView(bytes)
      view.toString should equal (new String(bytes))
    }

    "return its second element when at(1) is called" in {
      val bytes = Array[Byte](1, 2, 3, 4, 5)
      val view = ByteView(bytes)
